...
- [ ] A "⋯" menu on every item, holding what you can do TO it. Tasks have one already (2026-08-03: Rename, with delete and the rest joining it). **Projects should get the same** (idea 2026-08-03): rename, archive, delete, and later complete/reopen, colour and emoji — most naturally on the sidebar row when you hover it, mirroring how a task row works. Open when built: whether the project view's header carries one too, and what the Inbox is allowed to offer (renaming it is fine; deleting it means it simply comes back on the next start).
...
- [ ] Languages: the app can be switched easily between languages (idea 2026-08-03). Every piece of text the user reads comes from a language file instead of being written into the screens, and changing the language applies immediately, without restarting. It also covers the things that are not words: dates ("3 August" vs "3 de agosto"), which day the week starts on, and number formats. NOTE ON TIMING: this gets more expensive with every screen built, because each new hard-coded English sentence is one more to hunt down later — cheap habit meanwhile is to keep user-visible text grouped in one place per screen rather than scattered.
